...
test('MeshV2Service Batch Events', t => {
  ...
  t.test('processNextBroadcast does nothing when disconnected', st => {
    ...
    st.equal(service.pendingBroadcasts.length, queueLengthBefore, 'Queue should remain unchanged when disconnected')
    st.equal(service.batchStartTime, batchStartBefore, 'batchStartTime should remain unchanged when disconnected')
    st.equal(
