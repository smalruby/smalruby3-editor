...
test('Koshien Blocks', (t) => {
    ...
    t.test('falls back to MockClient when no endpoint is configured', (st) => {
        const blocks = new KoshienBlocks(createMockRuntime());
        st.equal(blocks._client._endpoint, undefined, 'no remote endpoint');
        st.equal(blocks._client, blocks._mockClient, 'using the MockClient');
        // mock readers still work (revealed via getMapArea)
        blocks.getMapArea({ POSITION: '5:1' });
        st.equal(blocks.map({ POSITION: '5:1' }), 0, 'mock map value (player space)');
        st.end();
    });
