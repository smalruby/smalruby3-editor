...
describe('RubyToBlocksConverter/Variables', () => {
    ...
    describe('Variable Scope Validation', () => {
        ...
        test('should error when changing global variable to instance variable', async () => {
            ...
            expect(converter.errors[0].text).toBe('"@global_variable", can\'t change variable scope. Delete the variable first, then recreate it with the correct scope.');
        ...
        test('should error when changing instance variable to global variable', async () => {
            ...
            const res = await converter.targetCodeToBlocks(mockTarget, code);
            expect(converter.errors).toHaveLength(1);
            expect(converter.errors[0].text).toBe('"$instance_variable", can\'t change variable scope. Delete the variable first, then recreate it with the correct scope.');
            expect(res).toBeFalsy();
        });
