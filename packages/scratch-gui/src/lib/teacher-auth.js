 ...
 * modal takes the button with it. It is rendered unconditionally instead of
 * only when One Tap fails to show, because the prompt-moment status methods
 * that used to drive that decision (`isNotDisplayed()` / `isSkippedMoment()`)
 * stop working once FedCM becomes mandatory.
 ...
 *   into. Without it only One Tap is offered.
 * @returns {Promise<string>} Google ID token
 */
export const loginWithGoogle = async ({ container } = {}) => {
