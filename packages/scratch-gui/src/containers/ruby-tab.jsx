...
const RubyTab = props => {
  ...
  const handlePreviewRubyScript = useCallback(async () => {
    ...
    if (rubyCode.modified) {
      const converter = await targetCodeToBlocks(vm, rubyCode.target, rubyCode.code, intl, { version: rubyVersion })
      if (!converter.result) {
        onShowAlert('convertRubyToBlocksError')
  ...
  return (
    <>
      ...
      <div className={styles.zoomControlsWrapper}>
        ...
        >
          <img alt="Rubyコードを画像として保存" className={styles.zoomIcon} draggable={false} src={cameraIcon} />
        </button>
