...
const GoogleDriveSaverHOC = function (WrappedComponent) {
  class GoogleDriveSaverComponent extends React.Component {
    ...
    scheduleAutoSave() {
      ...
      if (isGoogleDriveFile && this.state.autoSaveTimeoutId === null && this.state.saveDirectStatus !== 'saving') {
    ...
    async handleSaveDirectlyToGoogleDrive(isUserInitiated = true) {
      ...
      try {
        ...
        const isAuthError =
          ...
          (error &&
            error.message &&
            (error.message.toLowerCase().includes('auth') || error.message.toLowerCase().includes('unauthorized')))
  ...
  return RubyToBlocksConverterHOC(injectIntl(connect(mapStateToProps, mapDispatchToProps)(GoogleDriveSaverComponent)))
}
