...
2. 接続モーダル (`connection-modal`) が開く
3. **ホスト**として新しいグループを作るか、**メンバー**として既存グループ名で参加するかを選択
4. 接続後、`broadcast` ブロックでイベント送信、グローバル変数の自動同期、`mesh.sensor_value(name)` で他ノードの値読み取りが可能に
5. グループの最大接続時間は **35 分**（`MESH_MAX_CONNECTION_TIME_SECONDS`）
