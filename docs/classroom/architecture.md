...
- `GET /classrooms` は次の 3 つの和集合: owner 分（`teacherSub-index`）、課題単位の co-taught 分（課題の `coTeacherEmails` に `contains`）、**管理できるクラス（所有 + 共同管理）に属する課題**（クラスを列挙し、その `groupId` で絞り込み）。3 つ目は作成者を問わず拾うので、共同管理者にはオーナーが作った課題が、オーナーには共同管理者が作った課題が見える。DynamoDB はリスト属性も `groupId` も GSI 化していないため後ろ 2 つは Scan だが、**`contains(coTeacherEmails, :email) OR groupId IN (...)` の 1 Scan に統合**している（RCU はフィルタ適用「前」に読んだ項目に課金されるため、Scan 回数がそのまま課金に効く）。`IN` はオペランド 100 個が上限なので、管理クラスが 100 を超えるとその分だけチャンク分割される。各クラスは `role`（owner / co-teacher）を返し、フロントの「共同管理」バッジに使う。
