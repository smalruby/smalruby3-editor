...
- `GET /classrooms` は次の 3 つの和集合: owner 分（`teacherSub-index`）、課題単位の co-taught 分（課題の `coTeacherEmails` に `contains`）、**管理できるクラス（所有 + 共同管理）に属する課題**（クラスを列挙し、その `groupId` で絞り込み）。3 つ目は作成者を問わず拾うので、共同管理者にはオーナーが作った課題が、オーナーには共同管理者が作った課題が見える。DynamoDB はリスト属性も `groupId` も GSI 化していないため後ろ 2 つは Scan だが、**`contains(coTeacherEmails, :email) OR groupId IN (...)` の 1 Scan に統合**している（RCU はフィルタ適用「前」に読んだ項目に課金されるため、Scan 回数がそのまま課金に効く）。`IN` はオペランド 100 個が上限なので、管理クラスが 100 を超えるとその分だけチャンク分割される。各課題は `role`（owner / co-teacher）を返し、フロントの「共同管理」バッジに使う。
- **ページング**: DynamoDB の 1MB 上限もフィルタ適用「前」に効くため、`LastEvaluatedKey` を辿らないとテーブル肥大時にエラー無しで取りこぼす。一覧系の Query / Scan は共通ヘルパー（`queryAll` / `scanAll`）で最後のページまで辿る。対象は `GET /classrooms` / `GET /classroom-groups` の読み取りに加え、**列挙結果がそのまま書き込みを駆動する経路**（組への移行 `POST /classroom-groups/migrate`、クラス人数の課題への伝播、課題名の重複連番、トピックのカスケード）も含む — ここで取りこぼすと「一部だけ更新されない」状態になり、読み取りの取りこぼしより気付きにくい。無限ループ防止に `DDB_MAX_PAGES`（既定 25、不正値は既定に戻す）でページ数を打ち切り、打ち切り時は CloudWatch に warn を残す。単一クラス内に閉じた Query（メンバー・提出物・退室依頼）と `Limit` 付きの「上位 N 件」系はページングしない。
