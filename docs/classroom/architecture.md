...
- **ページング**: DynamoDB の 1MB 上限もフィルタ適用「前」に効くため、`LastEvaluatedKey` を辿らないとテーブル肥大時にエラー無しで取りこぼす。一覧系の Query / Scan は共通ヘルパー（`queryAll` / `scanAll`）で最後のページまで辿る。無限ループ防止に `DDB_MAX_PAGES`（既定 25）でページ数を打ち切り、打ち切り時は CloudWatch に warn を残す。
