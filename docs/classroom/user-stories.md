...
### 7. クラスを複数の先生で共同管理する (Issue #704)

同じ授業を複数の先生で担当する場合、クラスを作成した先生（owner）が、クラス詳細画面の「共同管理者」セクションから別の先生を **email** で招待できます。

- 招待された先生は次回ログイン時、ダッシュボードに該当クラスが「共同管理」バッジ付きで表示され、メンバー管理・提出返却・課題配信・クラス削除・共同管理者の追加/解除まで owner と同等に行えます。
...
- **クラス（組）単位で招待された共同管理者**は、そのクラスに属する課題を owner と同じように扱えます: 作成者が作った課題も課題一覧に出ますし、自分で新しい課題を作ることもできます。ただし**そのクラスの共同管理者リストの編集だけは owner 専用**です。
- クラスの所有者でも共同管理者でもない先生には、そのクラスは存在しないものとして扱われます（「クラスが見つかりません」）。
...
- **クラスに課題コンテンツが設定されていると、参加した瞬間にスタータープロジェクトが自動で開き、課題ページが表示される**（プログラム配付の手作業が不要になる）。編集中のプロジェクトがある場合は確認ダイアログを挟み、勝手に上書きしない。課題はステータス画面の「課題を見る」からいつでも開き直せる
