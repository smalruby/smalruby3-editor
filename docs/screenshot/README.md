...
- 形式: PNG
- 解像度: **書き出し倍率で高解像度化**（ブロックは SVG なので、倍率を上げるとベクタとして
  再描画されぼやけない）

### ブロックスクリーンショットの書き出し倍率

レイアウトはブロックの自然サイズ（`LOGICAL_SCALE = 1`）で固定し、ピクセル密度だけを
倍率で上げる。**`workspace.scale`（ユーザーの現在ズーム。既定 0.675）には依存しない**ので、
同じプログラムなら誰が撮っても同じサイズの画像になる。

| 経路 | 倍率 | 理由 |
|---|---|---|
| コードタブのカメラボタン（`downloadBlocksAsImage`） | `DOWNLOAD_EXPORT_SCALE = 4` | 教材・ワークシート・印刷での判読性 |
| クラス機能の提出 / 不具合報告（`renderBlocksToCanvas` 既定） | `DEFAULT_EXPORT_SCALE = 2` | アップロード / S3 保存サイズを抑える。Ruby タブの `PIXEL_RATIO = 2` と同等 |

倍率は呼び出し側が `{exportScale}` で上書きできる。巨大なプログラムでは
