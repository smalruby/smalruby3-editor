...
| `src/virtual-machine.js` | mesh v1 to v2 migration | mesh v1→v2 移行の統合（**同名マーカー 4 ペア**）: ① `serialization/smalruby-migration.js` からの import、② プロジェクトロード時の `options.migrateMeshV1ToV2` 分岐、③ `hasMeshV1Project` / `hasKoshienProject` / `migrateMeshV1InBackpackBlocks` / `migrateMeshV1InBackpackSprite` メソッド群、④ `shareBlocksToTarget` での stray v1 opcode 書き換え。mesh v1 サービス廃止 (#592) 後もプロジェクト/バックパックを壊さないための後方互換 |
| `src/extension-support/extension-manager.js` | builtin extension entries | `builtinExtensions` オブジェクトリテラル内の直書き 3 行（`mesh` / `meshV2` / `smalrubotS1`）。新規拡張はこの形式ではなく `smalruby-extensions.js` 経由で登録する（`.claude/rules/scratch-vm/development.md`） |

## 関連ファイル

マーカーで囲まれたコードが参照するファイル:
- `src/extension-support/smalruby-extensions.js` — extension-manager.js のマーカーから参照
- `src/serialization/smalruby-migration.js` — virtual-machine.js のマーカーから参照（mesh v1→v2 移行・koshien 検出）
